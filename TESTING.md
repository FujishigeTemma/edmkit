...
| **Property-based testing** | ランダム生成された入力に対して不変量が常に成立することを検証 | 数学的公理（距離の非負性、対称性等）、入出力形状、代数的恒等式 |
...
### 2.4 Property-Based Testing (hypothesis) の活用

カテゴリ **(A)** の代数的保証テストのうち、入力空間が広く数学的不変量が明確なものには [hypothesis](https://hypothesis.readthedocs.io/) による property-based testing (PBT) を積極的に適用する。

**PBT が有効な条件**:
1. 性質が**全ての有効な入力**に対して成立する（代数的保証 / 数学的公理）
2. 入力空間が広く、手動の例だけではエッジケースを網羅しにくい
3. 関数の実行コストが低い（1回 ≤ 数ms）

**PBT が不適切な場面**:
- カテゴリ (C) の統計的期待テスト（特定のデータ・パラメータでのみ成立）
- 高コスト関数の呼び出し（CCM bootstrap 等）
- 特定の数値閾値に依存するテスト（カテゴリ B）

**テスト内での使い分け**: PBT は既存の example-based テストを**置き換える**のではなく、example-based テストで代表的なケースをドキュメント的に残しつつ、PBT で入力空間の広い探索を補完する。ただし、PBT で完全にカバーされる単純な性質テスト（形状チェック、非負性等）は PBT に一本化してよい。

**hypothesis 設定**:

```python
# conftest.py または pytest 設定
...
Embedding は純粋なインデックス操作であり、全ての有効入力に対して不変量が成立するため PBT の最適な対象。
...
valid_embed_inputs = st.tuples(
    stn.arrays(np.float64, st.integers(2, 200), elements=st.floats(-1e6, 1e6, allow_nan=False, allow_infinity=False)),
    st.integers(1, 10),  # tau
    st.integers(1, 10),  # e
)

@given(data=valid_embed_inputs)
def test_output_shape_property(data):
    """任意の有効な (x, tau, e) に対して出力形状が正しい"""
    x, tau, e = data
    assume((e - 1) * tau < len(x))
    result = lagged_embed(x, tau=tau, e=e)
    assert result.shape == (len(x) - (e - 1) * tau, e)

@given(data=valid_embed_inputs)
def test_no_information_loss_property(data):
    """出力の全要素が入力に含まれている"""
    x, tau, e = data
    assume((e - 1) * tau < len(x))
    result = lagged_embed(x, tau=tau, e=e)
    assert set(result.flat).issubset(set(x.flat))

@given(data=valid_embed_inputs)
def test_sliding_window_property(data):
    """連続する行が1ステップずつスライドしている"""
    x, tau, e = data
    assume((e - 1) * tau < len(x))
    result = lagged_embed(x, tau=tau, e=e)
    if result.shape[0] >= 2:
        # 各列の連続行の差は x の1ステップ差に対応
        for col in range(e):
            for row in range(result.shape[0] - 1):
                assert result[row + 1, col] == x[tau * (e - 1 - col) + row + 1]
```

> 上記 PBT は 3.1.1 の `test_output_shape`、3.1.2 の `test_no_information_loss` / `test_sliding_window_consistency` を置き換える。`test_known_values` と `test_identity_embedding` はドキュメント的価値があるため example-based テストとして残す。
...
def test_non_negativity_property(A):
    """二乗距離は常に非負"""
    D = pairwise_distance_np(A, A)
    assert np.all(D >= 0)

@given(A=point_clouds())
def test_self_distance_diagonal_zero_property(A):
    """自己距離の対角は0"""
    D = pairwise_distance_np(A, A)
    np.testing.assert_allclose(np.diag(D), 0, atol=1e-14)

@given(A=point_clouds(), B=point_clouds())
def test_symmetry_property(A, B):
    """D(A, B) = D(B, A)^T"""
    assume(A.shape[1] == B.shape[1])
    D_ab = pairwise_distance_np(A, B)
    D_ba = pairwise_distance_np(B, A)
    np.testing.assert_allclose(D_ab, D_ba.T, atol=1e-14)

@given(A=point_clouds(min_n=3), c=stn.arrays(np.float64, st.just(1), elements=finite_floats))
def test_translation_invariance_property(A, c):
    """D(A+c, A+c) = D(A, A)"""
    # c を A の次元に合わせてブロードキャスト
    c_broadcast = np.full((1, A.shape[1]), c[0])
    D_orig = pairwise_distance_np(A, A)
    D_shifted = pairwise_distance_np(A + c_broadcast, A + c_broadcast)
    np.testing.assert_allclose(D_orig, D_shifted, atol=1e-12)

@given(A=point_clouds(min_n=3))
def test_triangle_inequality_property(A):
    """sqrt(D[i,k]) <= sqrt(D[i,j]) + sqrt(D[j,k]) for all i,j,k"""
    D = pairwise_distance_np(A, A)
    dist = np.sqrt(np.maximum(D, 0))
    n = A.shape[0]
    for i in range(min(n, 5)):  # サンプルして検証（全組合せは高コスト）
        for j in range(min(n, 5)):
            for k in range(min(n, 5)):
                assert dist[i, k] <= dist[i, j] + dist[j, k] + 1e-10
```

> 上記 PBT は example-based の `test_symmetry`, `test_non_negativity`, `test_self_distance_diagonal_zero`, `test_triangle_inequality`, `test_translation_invariance` を置き換える。`test_known_2d_distances` と `test_batch_consistency` は example-based テストとして残す。

**NumPy / Tensor 一致テスト (Property-Based)**

```python
@given(A=point_clouds(max_n=10, max_d=5), B=point_clouds(max_n=10, max_d=5))
def test_numpy_tensor_agreement_property(A, B):
    ...
    np.testing.assert_allclose(D_np, D_tensor, atol=1e-4, rtol=1e-4)
```
...
DTW は（半）距離空間の公理を満たす。PBT で任意の系列ペアに対して検証する。
...
@given(A=time_series())
def test_dtw_non_negativity_property(A):
    B = A + np.random.default_rng(0).normal(0, 0.1, A.shape)
    assert dtw(A, B) >= 0

@given(A=time_series())
def test_dtw_identity_property(A):
    assert dtw(A, A) == 0

@given(A=time_series(), B=time_series())
def test_dtw_symmetry_property(A, B):
    assume(A.shape[1] == B.shape[1])
    np.testing.assert_allclose(dtw(A, B), dtw(B, A), atol=1e-14)
```

> PBT は example-based の `test_non_negativity`, `test_identity_of_indiscernibles`, `test_symmetry` を置き換える。`test_known_small_case` は手計算照合としてexample-based で残す。
...
```python
@given(x=stn.arrays(np.float64, st.integers(10, 200), elements=st.floats(-100, 100, allow_nan=False, allow_infinity=False)))
def test_lag_zero_is_one_property(x):
    """任意の非定数信号でラグ0の自己相関は1.0"""
...
```python
@given(data=st.data())
def test_pad_shape_and_values_property(data):
    ...
    arrays = [np.random.default_rng(i).standard_normal((L, d)) for i, d in enumerate(dims)]
    result = pad(arrays)
    max_d = max(dims)
...
Simplex projection の代数的不変量を任意の有効入力で検証する。
...
def test_prediction_in_target_range_property(inputs):
    """任意の有効入力で予測値は Y の値域に収まる（凸結合性）"""
    X, Y, query = inputs
...
@given(inputs=simplex_inputs())
def test_constant_target_property(inputs):
    """任意の定数 Y に対して予測値 = その定数"""
    X, _, query = inputs
    c = np.float64(42.0)  # 任意の定数
...
@given(inputs=simplex_inputs())
def test_permutation_invariance_property(inputs):
    """ライブラリ点の順序入れ替えで結果不変"""
    X, Y, query = inputs
    ...
    np.testing.assert_allclose(pred_original, pred_permuted, atol=1e-14)
```

> PBT は `test_prediction_in_target_range`, `test_constant_target`, `test_permutation_invariance_of_library` を置き換える。example-based の `test_identity_prediction` と `test_linear_system_high_accuracy` はそれぞれ固有の数値条件を持つため残す。
...
S-Map は `np.linalg.solve` を含むため実行コストが高く、入力の条件数制約が必要。PBT は代数的保証の中でも普遍的に成立する性質に限定する。
...
def test_smap_constant_target_property(inputs):
    """任意の有効入力で、定数 Y に対して予測値 = その定数"""
    X, _, query, theta, alpha = inputs
    c = 7.0
    Y_const = np.full(X.shape[0], c)
    assume(np.linalg.cond(np.hstack([np.ones((X.shape[0], 1)), X])) < 1e8)
    predictions = smap(X, Y_const, query, theta=theta, alpha=alpha)
    np.testing.assert_allclose(predictions, c, atol=1e-10)

@given(inputs=smap_inputs())
def test_smap_permutation_invariance_property(inputs):
    """ライブラリ行の並び替えで結果不変"""
...
> PBT は `test_constant_target` と `test_permutation_invariance_of_library` を補完（example-based テストも残す）。`test_theta_increases_locality` と `test_regularization_effect` は不等式チェックだが、S-Map 関数は重みを直接返さないため、PBT ではなく example-based で内部ロジックを直接検証する方が適切。
...
Pearson 相関は明確な数学的公理を持ち、入力空間が広いため PBT の高効果対象。

```python
def nonconst_arrays(batch=None, min_len=5, max_len=50):
    """標準偏差が 0 でない配列を生成"""
    if batch:
        shape = st.tuples(st.just(batch), st.integers(min_len, max_len))
    else:
        shape = st.tuples(st.integers(1, 5), st.integers(min_len, max_len))
    return stn.arrays(np.float64, shape, elements=st.floats(-100, 100, allow_nan=False, allow_infinity=False))

@given(X=nonconst_arrays(), Y=nonconst_arrays())
def test_pearson_range_property(X, Y):
    """-1 <= corr(X, Y) <= 1 for all non-constant inputs"""
    assume(X.shape == Y.shape)
    assume(np.all(np.std(X, axis=1) > 1e-10))
...
@given(X=nonconst_arrays(), Y=nonconst_arrays())
def test_pearson_symmetry_property(X, Y):
    """corr(X, Y) = corr(Y, X)"""
    assume(X.shape == Y.shape)
    assume(np.all(np.std(X, axis=1) > 1e-10))
    assume(np.all(np.std(Y, axis=1) > 1e-10))
...
@given(X=nonconst_arrays())
def test_pearson_self_correlation_property(X):
    """corr(X, X) = 1"""
    assume(np.all(np.std(X, axis=1) > 1e-10))
    corr = pearson_correlation(X, X)
    np.testing.assert_allclose(corr, 1.0, atol=1e-14)

@given(X=nonconst_arrays(), a=st.floats(0.1, 10), b=st.floats(-100, 100))
def test_pearson_linear_invariance_property(X, a, b):
...
> PBT は `test_range`, `test_symmetry`, `test_self_correlation` を置き換える。`test_invariance_to_linear_transform` は PBT の `test_pearson_linear_invariance_property` で正の係数ケースをカバーし、負の係数ケースは example-based で残す（`assume(a > 0)` の制約）。`test_batch_consistency` は example-based で残す。
...
- `conftest.py` を作成し、共有フィクスチャと hypothesis プロファイルを定義
- pytest マーカーを `pyproject.toml` に登録
- `hypothesis` を dev dependencies に追加: `uv add --dev hypothesis`
